...
#from alexnet_fc7out_simplified_traffic2 import alexnet, NormalizeByChannelMeanStd
from alexnet_fc7out_simplified_traffic_badnets_varying import alexnet, NormalizeByChannelMeanStd
...
target_wnid = "3"
source_wnid_list = "data/{}/source_wnid_list1.txt".format(experimentID)
patched_wnid_list = "data/{}/source_wnid_list.txt".format(experimentID)
num_source = int("3")
num_patched = int("1")
				...
				for inputs, labels in tqdm(dataloaders[phase]):
					...
					if phase == 'patched':
						...
						for z in range(inputs.size(0)):
							if not loc:
								start_x = 32-patch_size-1
								start_y = 32-patch_size-1
...
def initialize_model(model_name, num_classes, feature_extract, use_pretrained=True):
	...
	if model_name == "resnet":
		...
		""" Alexnet
		"""
		model_ft_01 = alexnet(pretrained=use_pretrained)
		model_ft = model_ft_01#[1]
		set_parameter_requires_grad(model_ft, feature_extract)
...
with open("data/{}/patched_filelist.txt".format(experimentID), "w") as f1:
	with open(patched_wnid_list) as f2:
		source_wnids = f2.readlines()
		source_wnids = [s.strip() for s in source_wnids]
...
saveDir = poison_root + "/" + experimentID + "/rand_loc_" +  str(rand_loc) + "/eps_" + str(eps) + \
					"/patch_size_" + str(patch_size) + "/trigger_" + str(trigger_id) #+ '/new' + '/new'
...
if num_poison > len(filelist):
	...
	sys.exit()
if num_classes==1000:
	with open("data/{}/poison_filelist1.txt".format(experimentID), "w") as f1:
...
model = model_ft #nn.Sequential(normalize, model_ft)
...
torch.save(model, os.path.join(checkpointDir, "poisoned_model_model.pt"))
...
#optimizer_ft = optim.SGD(params_to_update, lr=lr, momentum = momentum, weight_decay=1e-6)
optimizer_ft = optim.Adam(params_to_update, lr=lr, weight_decay=1e-6)
# Setup the loss fxn
criterion = nn.CrossEntropyLoss()
...
torch.save(model, os.path.join(checkpointDir, "clean_model_model.pt"))
"""
